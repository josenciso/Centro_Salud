window.addEventListener("DOMContentLoaded", () => {
  ...
  //variables
  let num = document.getElementById("num"); //dni
  ...
  let medicos = document.getElementById("flexRadioDefault1");
  let administrativos = document.getElementById("flexRadioDefault2");
  ...
  //ASIGNAR SEGUN ROL CONSULTA ESCRIBIR placeholder de numero colegiado o adminis
  ...
  formulario.addEventListener("click", function () { //primero escoger 
  ...
  num.addEventListener("blur", function (e) {
    ...
    let reg = regex.test(num.value.toLowerCase());
    let regex1 = regex_num.test(num.value);
      ...
      if ((medicos.checked == true && administrativos.checked == false) || (medicos.checked == false && administrativos.checked == true)) {
        ...
        function recibir() { // ver si ya esta registrado el n colegiado o n de administrativo
          ...
          if (val) {
            swal(
              "ATENCION",
              "Este numero de registro ya esta siendo usado",
              "warning"
            );
            num.className = "ko";
            val = false;
