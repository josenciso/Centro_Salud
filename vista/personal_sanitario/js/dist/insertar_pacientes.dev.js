...
window.addEventListener("DOMContentLoaded", function () {
  ...
  var dni_regex = /^[0-9]{8,8}[A-Za-z]$/; //dni aunque

  var nombre_regex = /^[A-Z]{1}.* [A-Z]{1}.*/;
  var pass_regex = /^[a-zA-Z0-9]{8}$/; // 8 maximo, solo letras y numeros

  var regex_email = /^(([^<>()[\]\.,;:\s@\"]+(\.[^<>()[\]\.,;:\s@\"]+)*)|(\".+\"))@(([^<>()[\]\.,;:\s@\"]+\.)+[^<>()[\]\.,;:\s@\"]{2,})$/; //const regex_email = /^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,3,4})+$/;
  ...
  var direccion_error = document.getElementById("nombre_error"); //direccion_error

  var ciudad = document.getElementById("ciudad"); //ciudad

  var ciudad_error = document.getElementById("ciudad_error"); //ciudad_error
  ...
  var fecha = document.getElementById("fecha"); //fecha

  var fecha_error = document.getElementById("fecha_Error"); //fecha_error
  ...
  var nun = 1;
  var val = false;
  var val_1 = false; //***validando DNI */

  dni.addEventListener("blur", function (e) {
    dni.className = "form-control";
    var reg = regex.test(dni.value.toLowerCase());
    ...
    if (reg) {
      ...
      var recibir = function recibir() {
        if (conexion.readyState == 4 && conexion.status == 200) {
          datos = JSON.parse(conexion.responseText); // console.log(datos);

          longitud = datos.length;

          for (i = 0; i < longitud; i++) {
            if (datos[i].title == dni.value.toUpperCase()) {
              val = true;
              i = 100;
            }
          }
          ...
          if (val) {
            swal("ATENCION", "Este numero de dni ya esta siendo usado", "warning");
            dni.className = "ko";
            val = false;
          } else {
            dni.className = "ok";
  ...
  nombre.addEventListener("blur", function (e) {
    ...
    if (reg) {
      contador++; //console.log("Error_palabra falsa");

      nombre.className = "error";
      nombre_error.innerHTML = "PALABRA NO PERMITIDA"; //
  ...
  direccion.addEventListener("blur", function (e) {
    ...
    var reg = regex.test(direccion.value.toLowerCase()); // let reg_dni = dni_regex.test(nombre.value.toLowerCase());

    if (reg) {
      contador++; // console.log("Error_palabra falsa");

      direccion.className = "error";
      direccion_error.innerHTML = "PALABRA NO PERMITIDA"; //
  ...
  ciudad.addEventListener("blur", function (e) {
    ...
    var reg = regex.test(ciudad.value.toLowerCase()); // let reg_dni = dni_regex.test(nombre.value.toLowerCase());

    if (reg) {
      contador++;
      console.log("Error_palabra falsa");
      ciudad.className = "error";
      ciudad_error.innerHTML = "PALABRA NO PERMITIDA"; //
  ...
  medico.addEventListener("focus", function (e) {
    //AL ENTRAR AL FOCO, LIMPIA LOS ERRORES
    medico.className = "form-control"; //medico.innerHTML = ""
  ...
  verifica.addEventListener("click", function () {
    if (dni.className == "ok" && nombre.className == "ok" && telefono.className == "ok" && contraseña.className == "ok" && email.className == "ok" && contraseña_1.className == "ok" && direccion.className == "ok" && medico.className == "ok" && ciudad.className == "ok" && fecha.className == "ok") {
      console.log("ok todo");
  ...
  function cancelar_envio() {
    // si vuevle a entrar en el input boton envio rojo
    enviar.setAttribute("disabled", "disabled");
    enviar.className = "col-12 btn btn-danger";
  }
