...
var moda = document.getElementById("evento");
document.addEventListener("DOMContentLoaded", function () {
  ...
  var calendar = new FullCalendar.Calendar(calendarEl, {
    ...
    dateClick: function dateClick($e) {
      ...
      var formatted_date;
      var date = new Date();
      var dia01 = date.getDate(); //formatea la fecha a si es 1 le pone 01
      ...
      if (formatDate(date) <= dia) {
        //si el dia es igual o mayor mostramos cita 
        //console.log("hola");
        $("#evento").modal('show'); //console.log($('#hora').val)
  ...
  $("#btnagregar").click(function () {
    var datos = datos_del_formulario(); //datos recogidos de los input
    // mostrar_horas1();
    ...
    $.ajax({
      ...
      success: function success(msg) {
        ...
        if (msg == 1) {
          ...
          $("#evento").modal('hide'); //segundo cambio 

          events: //"voolvemos a cargar el calendario con la nueva cita 
          "http://localhost/PROYECTO_FINAL/calendario/eventos/eventos.php?accion=listar"; // window.location.assign("http://localhost:3000/vista/personal_sanitario/gestionar_pacientes.php");
        ...
        } else swal("Ocurrio un error,Verifqiue citas existentes. Avise al administrador  en caso de continuar el error", "error");
      },
  ...
  function verificar_fechas(date_click) {
    var horas = [];
    var contador = $("#contador").val(); //variable del tamaño del array php
    ...
    for (var i = 0; i < contador; i++) {
      ...
      if ($("#verificar_fecha" + i).val() == date_click) {
        //si hay coincidencia con la fecha seleccionada 
        //horas.push($("#verificar_hora" + i).val()); //
        //busco hora
        //console.log("hola buscando horas");
        //console.log($("#verificar_hora" + i).val());
        if ($("#verificar_hora" + i).val() == $("#h1").val()) {
          // si coinciden las fechas no disponibles
          $('#h1').attr('disabled', 'disabled'); // console.log("para las 9");
        }

        if ($("#verificar_hora" + i).val() == $("#h2").val()) {
          // si coinciden las fechas no disponibles
          $('#h2').attr('disabled', 'disabled'); // console.log("para las 10");
        }

        if ($("#verificar_hora" + i).val() == $("#h3").val()) {
          // si coinciden las fechas no disponibles
          $('#h3').attr('disabled', 'disabled'); // console.log("para las 11");
        }

        if ($("#verificar_hora" + i).val() == $("#h4").val()) {
          // si coinciden las fechas no disponibles
          $('#h4').attr('disabled', 'disabled'); // console.log("para las 12");
        }

        if ($("#verificar_hora" + i).val() == $("#h5").val()) {
          // si coinciden las fechas no disponibles
          $('#h5').attr('disabled', 'disabled'); // console.log("para las 13");
        }
      }
    }
  ...
  function restablecer_horas() {
    var texto = $("#texto").val(); //console.log(texto.length);
